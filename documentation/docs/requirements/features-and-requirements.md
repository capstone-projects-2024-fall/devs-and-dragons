...
* The GPT component of the game must be able to grade answers based on correct and incorrect user(s) input:
  ...
  * GPT must not mark an incorrect answer (such as a blank answer, an answer with content irrelavant to the question, or an answer without an actual algorithm) as a valid solution. Inaccurately marking answers as valid would let users wrongly move onto the next question.
